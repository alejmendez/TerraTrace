...
const form = useForm({
  ...
  is_commercial: Boolean(props.data?.is_commercial),
});

const submitHandler = () => form.put(route('category_products.update', props.data.id));
</script>

<template>
  <AuthenticatedLayout title="Editar categoría de producto">
    <CollectionPageHeader
      title="Editar categoría de producto"
      :description="`Modifica la categoría ${data.name}.`"
