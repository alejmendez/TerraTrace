...
const reloadList = (extra = {}) => {
  ...
  router.get(route('security_equipments.index', buildParams()), {
...
const setPage = (page) => {
  ...
  router.get(route('security_equipments.index', { ...buildParams(), page }), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    preserveScroll: true,
  });
...
const setPerPage = (perPage) => {
  router.get(route('security_equipments.index', { ...buildParams(), per_page: perPage, page: 1 }), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    preserveScroll: true,
  });
...
const deleteRecord = () => {
  ...
  const name = recordToDelete.value.name || 'el equipo de seguridad';
  recordToDelete.value = null;

  router.delete(route('security_equipments.destroy', id), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    onSuccess: () => notify(`Se eliminó ${name}.`),
    onError: () => notify('No fue posible eliminar el equipo.', 'error'),
  });
};

const totalEquipos = computed(() => formatNumber(props.summary?.equipments || 0, 0));
...
<template>
  <AuthenticatedLayout title="Equipos de seguridad">
    <CollectionPageHeader
      title="Equipos de seguridad"
      description="Catálogo de equipos de seguridad, fecha de compra y registro de mantención."
      :action-route="canCreate ? route('security_equipments.create') : ''"
      action-label="Nuevo equipo"
    />

    <section class="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-2" aria-label="Resumen de equipos">
      <CollectionMetricCard icon="agriculture" label="Equipos registrados" :value="totalEquipos" />
    ...
    <section class="relative rounded-xl border border-[#e1e9e3] bg-white shadow-[0_3px_14px_rgba(24,57,39,0.045)]">
      <div class="flex flex-col gap-3 border-b border-[#e9efea] p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 class="text-xl font-bold text-[#102f27]">Equipos registrados</h2>
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-start justify-between gap-3">
              ...
              <CollectionActionMenu
                :edit-route="canEdit ? route('security_equipments.edit', record.id) : ''"
      ...
      <div v-else class="p-12 text-center">
        ...
        <h3 class="mt-3 font-bold text-[#102f27]">No se encontraron equipos</h3>
        <p class="mt-1 text-sm text-[#61716c]">Ajusta la búsqueda o crea un nuevo equipo.</p>
      </div>
    ...
    <CollectionConfirmDialog
      ...
      :message="`Eliminarás el equipo de seguridad ${recordToDelete?.name || ''}. Esta acción no se puede deshacer.`"
