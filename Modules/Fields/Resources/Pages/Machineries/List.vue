...
const reloadList = (extra = {}) => {
  ...
  router.get(route('machineries.index', buildParams()), {
...
const setPage = (page) => {
  ...
  router.get(route('machineries.index', { ...buildParams(), page }), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    preserveScroll: true,
  });
...
const setPerPage = (perPage) => {
  router.get(route('machineries.index', { ...buildParams(), per_page: perPage, page: 1 }), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    preserveScroll: true,
  });
...
const deleteRecord = () => {
  ...
  const name = recordToDelete.value.name || 'la maquinaria';
  ...
  router.delete(route('machineries.destroy', id), {
    ...
    onError: () => notify('No fue posible eliminar la maquinaria.', 'error'),
  });
};

const totalMaquinarias = computed(() => formatNumber(props.summary?.machineries || 0, 0));
...
<template>
  <AuthenticatedLayout title="Maquinarias">
    <CollectionPageHeader
      title="Maquinarias"
      description="Catálogo de maquinaria, fecha de compra y registro de mantención."
      :action-route="canCreate ? route('machineries.create') : ''"
      action-label="Nueva maquinaria"
    />

    <section class="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-2" aria-label="Resumen de maquinarias">
      <CollectionMetricCard icon="agriculture" label="Maquinarias registradas" :value="totalMaquinarias" />
    ...
    <section class="relative rounded-xl border border-[#e1e9e3] bg-white shadow-[0_3px_14px_rgba(24,57,39,0.045)]">
      <div class="flex flex-col gap-3 border-b border-[#e9efea] p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 class="text-xl font-bold text-[#102f27]">Maquinarias registradas</h2>
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <label class="relative block sm:flex-1 sm:min-w-[200px]">
            <span class="sr-only">Buscar maquinarias</span>
          ...
          <select
            ...
            :value="query.sort"
            aria-label="Ordenar maquinarias"
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-start justify-between gap-3">
              ...
              <CollectionActionMenu
                :edit-route="canEdit ? route('machineries.edit', record.id) : ''"
      ...
      <div v-else class="p-12 text-center">
        ...
        <h3 class="mt-3 font-bold text-[#102f27]">No se encontraron maquinarias</h3>
        <p class="mt-1 text-sm text-[#61716c]">Ajusta la búsqueda o crea una nueva maquinaria.</p>
      </div>
    ...
    <CollectionConfirmDialog
      ...
      :message="`Eliminarás la maquinaria ${recordToDelete?.name || ''}. Esta acción no se puede deshacer.`"
