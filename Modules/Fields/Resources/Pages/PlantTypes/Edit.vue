...
const submitHandler = () => form.put(route('plant_types.update', props.data.id));
</script>

<template>
  <AuthenticatedLayout title="Editar tipo de planta">
    <CollectionPageHeader
      title="Editar tipo de planta"
      :description="`Modifica el nombre del tipo ${data.name}. El slug se regenera automáticamente.`"
    ...
    >
      ...
      <div class="mt-6 flex items-center justify-end gap-3 border-t border-[#e7eee8] pt-4">
        ...
        >
          {{ form.processing ? 'Guardando...' : 'Actualizar tipo' }}
        </button>
      </div>
    </form>
  </AuthenticatedLayout>
