...
const reloadList = (extra = {}) => {
  ...
  router.get(route('plant_types.index', buildParams()), {
...
const setPage = (page) => {
  ...
  router.get(route('plant_types.index', { ...buildParams(), page }), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    preserveScroll: true,
  });
...
const setPerPage = (perPage) => {
  router.get(route('plant_types.index', { ...buildParams(), per_page: perPage, page: 1 }), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    preserveScroll: true,
  });
...
const deleteRecord = () => {
  ...
  const name = recordToDelete.value.name || 'el tipo de planta';
  recordToDelete.value = null;

  router.delete(route('plant_types.destroy', id), {
    only: ['records', 'meta', 'summary'],
    preserveState: true,
    onSuccess: () => notify(`Se eliminó ${name}.`),
    onError: () => notify('No fue posible eliminar el tipo de planta.', 'error'),
  });
};

const totalTipos = computed(() => formatNumber(props.summary?.plant_types || 0, 0));
const totalConPlantas = computed(() => formatNumber(props.summary?.with_plants || 0, 0));
...
<template>
  <AuthenticatedLayout title="Tipos de planta">
    <CollectionPageHeader
      title="Tipos de planta"
      description="Define y administra los tipos de planta disponibles en el sistema."
      :action-route="canCreate ? route('plant_types.create') : ''"
      action-label="Nuevo tipo"
    />

    <section class="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-2" aria-label="Resumen de tipos">
      <CollectionMetricCard icon="potted_plant" label="Tipos registrados" :value="totalTipos" />
      <CollectionMetricCard icon="potted_plant" label="Con plantas asociadas" :value="totalConPlantas" />
    </section>
    ...
    <section class="relative rounded-xl border border-[#e1e9e3] bg-white shadow-[0_3px_14px_rgba(24,57,39,0.045)]">
      <div class="flex flex-col gap-3 border-b border-[#e9efea] p-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 class="text-xl font-bold text-[#102f27]">Tipos de planta</h2>
          <p class="mt-1 text-sm text-[#61716c]">Busca por nombre o slug del tipo.</p>
        </div>
        <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <label class="relative block sm:flex-1 sm:min-w-[200px]">
            <span class="sr-only">Buscar tipos</span>
      ...
      <div v-if="records.length" class="grid gap-3 p-4 md:grid-cols-2">
        ...
        >
          <span class="flex size-16 shrink-0 items-center justify-center rounded-lg bg-[#e7f3e9] text-[#17663a]" aria-hidden="true">
            <CollectionIcon name="potted_plant" :size="36" />
          </span>
          <div class="min-w-0 flex-1">
            <div class="flex items-start justify-between gap-3">
              ...
              <CollectionActionMenu
                :edit-route="canEdit ? route('plant_types.edit', record.id) : ''"
            ...
            <dl class="mt-4 grid grid-cols-1 gap-2 border-t border-[#e7eee8] pt-3 text-sm">
              <div>
                <dt class="text-[#75847d]">Plantas asociadas</dt>
                <dd class="mt-1 font-semibold text-[#284238]">{{ record.plants_count ?? 0 }}</dd>
              </div>
            </dl>
          </div>
        </article>
      ...
      <div v-else class="p-12 text-center">
        ...
        <h3 class="mt-3 font-bold text-[#102f27]">No se encontraron tipos</h3>
        <p class="mt-1 text-sm text-[#61716c]">Ajusta la búsqueda o crea un nuevo tipo.</p>
      </div>
    ...
    <CollectionConfirmDialog
      ...
      :message="`Eliminarás el tipo ${recordToDelete?.name || ''}. Esta acción no se puede deshacer.`"
