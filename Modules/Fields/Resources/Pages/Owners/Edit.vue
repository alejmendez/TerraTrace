...
const submitHandler = () => form.put(route('owners.update', props.data.id));
</script>

<template>
  <AuthenticatedLayout title="Editar propietario">
    <CollectionPageHeader
      title="Editar propietario"
      :description="`Modifica los datos del propietario ${data.name}.`"
    ...
    >
      ...
      <div class="mt-6 flex items-center justify-end gap-3 border-t border-[#e7eee8] pt-4">
        ...
        >
          {{ form.processing ? 'Guardando...' : 'Actualizar propietario' }}
        </button>
      </div>
    </form>
  </AuthenticatedLayout>
